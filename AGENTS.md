...
**푸시 알림으로 결과를 보내는 방안은 검토 후 보류했다.** 이 서비스는 웹이고 manifest·service
worker·web-push가 전부 없으며, iOS Safari의 Web Push는 사용자가 홈 화면에 추가해야만 동작한다
— 주 사용 맥락이 "인스타 보다가 링크 복사해서 돌아오기"라 대부분 iOS Safari이므로, 푸시를
