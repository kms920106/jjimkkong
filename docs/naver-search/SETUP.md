...
## 4-1. 블로그 검색도 같은 키로 동작한다 (실측 확인)

장소 시트의 "블로그 리뷰"(`lib/ingest/place-blog.ts`)는 **위의 지역검색 키 쌍을 그대로
쓴다.** 2026-08-30에 라이브 엔드포인트로 확인했다 — `/search/v1/blog`에 같은 헤더로
요청해 200과 정상 결과를 받았다.

이게 당연하지 않은 이유는 이미지 검색이 정반대이기 때문이다. `NAVER_IMAGE_CLIENT_ID`는
별도 애플리케이션을 요구하고, 그 사실이 코드 세 군데에 적혀 있다. **네이버는 API를
애플리케이션별로 켜므로**, 한 키 쌍이 여러 `/search/v1/*`에 닿는지는 그 애플리케이션에
어떤 API를 체크해 두었는지에 달렸다. 이 저장소가 쓰는 애플리케이션에는 지역 검색과
블로그 검색이 둘 다 켜져 있다.

새 환경에서 401 `요청한 API는 이 Application에서 활성화되어 있지 않습니다`가 뜨면,
1번 절차의 **사용할 API**에서 **블로그 검색**을 추가로 체크한다. 코드나 환경변수를
바꿀 일이 아니다.

블로그 검색 API의 상세 레퍼런스는 [BLOG-SEARCH.md](BLOG-SEARCH.md) 참고.
**정렬은 반드시 `date`다** — 기본값 `sim`은 장소 이름에 대해 무관한 옛날 글을 돌려준다
(근거는 루트 AGENTS.md의 `PlaceBlog` 문단).
