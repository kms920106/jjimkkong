...
무료 할당량은 프로젝트당 하루 10,000 units이고, 이 앱이 쓰는 호출은 둘 다 1 unit이다 —
설명 전문을 가져오는 `videos.list`와 채널 아바타를 가져오는 `channels.list`. **즉 유튜브
링크 하나를 새로 저장할 때 2 units을 쓴다.** 같은 링크를 다시 저장하면 저장된 `Post`를
그대로 쓰므로 추가 비용이 없다(루트 [AGENTS.md](../../AGENTS.md)의 "게시물과 찜은 다른
것이다" 참고). 이 서비스 규모에서는 넉넉하지만, 여러 프로젝트에서 같은 Google Cloud
프로젝트를 공유하면 할당량을 나눠 쓰게 된다는 점은 알아 둔다.
