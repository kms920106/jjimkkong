...
걷어내던 장치가 없어졌으므로 인가는 전부 라우트 핸들러의 `requireMember()` 한 곳에서만
일어난다. 페이지에 `requireMember()`를 다시 넣으면 "로그인 없이 접속 가능"이라는 전제가
깨지고, 미들웨어를 되살리면 세션이 만료된 `fetch()`가 401 대신 로그인 페이지 HTML을
받아 `res.json()`이 깨진다.

**페이지가 비어 보이는 것과 막히는 것은 다르다.** 로그아웃 상태의 홈은 핀도 목록도
비어 있지만, 그건 `getMember()`가 null이라 조회할 `memberId`가 없기 때문이다. 저장·삭제·
설정 변경은 그대로 401이다.
