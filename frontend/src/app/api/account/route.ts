 ...
 *
 * No stored data is deleted. The Member row, its AuthIdentity rows, and
 ...
 *
 *   1. requireMember()/getMember() reject a withdrawn profile, so no request can
 ...
 *
 * Ownership is the session itself: requireMember() resolves the cookie to exactly
 * one Member and that is the only row this route can reach, so there is no
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.info("Account withdrawn:", { memberId: user.id });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // account, relying entirely on the requireMember() check to hold. Dropping
      // the rows revokes every browser at once, which is the property the
      // database-backed session design exists for.
      await tx.session.deleteMany({ where: { memberId: user.id } });
    });
