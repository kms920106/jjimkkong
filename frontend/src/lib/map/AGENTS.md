...
링크 걸 place id를 주지 않는다. 네이버 지역검색의 `link`가 있긴 하지만 그건 지도 페이지가
아니라 업체가 등록해 둔 홈페이지(인스타 프로필·예약 페이지·빈 값 …)라, `Place.naverLink`로
저장하던 것을 읽는 곳이 없다는 게 확인되어 컬럼째 삭제했다. 되살리지 말 것.
